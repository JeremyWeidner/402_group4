...
ggplot(aes(x=CarPrice_Added$enginesize), data = CarPrice) + 
  geom_histogram(aes(y = ..density..), color = "grey30", fill = "white") +
  geom_density(alpha = 0.2, fill = "antiquewhite3") + 
  labs(title = "Histogram of Engine Size without Transformation")

#Transformation (Engine size is right skewed.)
library(car)
symbox(~enginesize, data = CarPrice_Added)

#log transformation
log_eng <- log(CarPrice_Added$enginesize)
...
library(forecast)
lambda.e <-BoxCox.lambda(CarPrice_Added$enginesize)
lambda.e #-0.9999242
boxcox_enginesize<-CarPrice_Added$enginesize^lambda.e

ggplot(aes(x=boxcox_eng), data = CarPrice) + 
  geom_histogram(aes(y = ..density..), color = "grey30", fill = "white") +
  geom_density(alpha = 0.2, fill = "antiquewhite3") + 
  labs(title = "Histogram: Predictor Variable Engine Size (boxcox transformation)")

#checking sknewness
skewness(CarPrice_Added$enginesize) #1.919245
skewness(log_eng) #0.8453149
skewness(boxcox_enginesize) #0.03646001 (boxcox transformation would be the best transformation.)
...
# Model for Presentation (FOR NOW)
```{r}
#all variables, no transformations on numeric
model.a <- lm(price ~ ChaChing + carbody + fueltype + enginelocation + cylindernumber + car_volume + curbweight + enginesize + stroke + peakrpm + average_mpg, data=CarPrice_Added)
summary(model.a) #93.93%
plot(model.a)
vif(model.a)

#remove fueltype, stroke, average_mpg
model.b <- lm(price ~ ChaChing + carbody + enginelocation + cylindernumber + car_volume + curbweight + enginesize +  peakrpm , data=CarPrice_Added)
summary(model.b) #93.92%
plot(model.b)
vif(model.b) #12.12 curbweight and 11.55 for enginesize

# remove curbweight
model.c <- lm(price ~ ChaChing + carbody + enginelocation + cylindernumber + car_volume + enginesize +  peakrpm , data=CarPrice_Added)
summary(model.c) #93.36%
plot(model.c)
vif(model.c)

# add interaction term
model.d <- lm(price ~ ChaChing:car_volume + carbody + enginesize + enginelocation + cylindernumber  +  peakrpm , data=CarPrice_Added)
...
scatterplotMatrix(~car_volume + curbweight + peakrpm + stroke + average_mpg + boxcox_enginesize, data = CarPrice_Added, main="Scatter plot matrix for numerical variables",var.labels = c("car_volume","curbweight","peakrpm","stroke","average_mpg","enginesize"))

sub1 <- cbind(CarPrice_Added[,c("car_volume","curbweight","peakrpm","stroke","average_mpg")],boxcox_enginesize)
cor(sub1, use = "complete.obs")
chart.Correlation(sub1)
#We can't use some variables together since some of the correlations are too high.
```
